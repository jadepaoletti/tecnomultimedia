class Boton {
  ...
  void pasarPantalla() {
    if (isMouseOver() && juego.pantalla == 0) {
      juego.pantalla = 1;
      juego.reiniciarIndicadores();
      juego.reiniciarObstaculos();
    } else if (isMouseOver() && juego.pantalla == 2) { // Pantalla Victoria
      juego.pantalla = 0;
    } else if (isMouseOver() && juego.pantalla == 3) { // Pantalla Derrota
      juego.pantalla = 0;
    }
  }
}
